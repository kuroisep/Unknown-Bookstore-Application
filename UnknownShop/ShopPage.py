...
class Shop_main_screen:
    ...
    def infomationPage(self): # ข้อมูลหน้า info       #1
        ...
        if self.user != []:
             ## USERNAME
            self.inner_infomation.create_text(150, 50, anchor=NW, text='Username : {}'.format(self.user[0][1]),font=('Verdana',15))
            ## NAME
            self.inner_infomation.create_text(150, 100, anchor=NW, text='Name : ')
            self.username_entry = Entry(self.inner_infomation,font=('Verdana',10))
            self.username_entry.insert(0,self.user[0][3])
            self.username_entry.config(state=DISABLED)
            self.inner_infomation.create_window(290,110,window=self.username_entry)
            ##LASTNAME
